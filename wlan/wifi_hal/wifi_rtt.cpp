...
public:
    ...
    virtual int create() {
        ...
        for (unsigned i = 0; i < mRttConfigs.size(); ++i) {
            ...
            result = mMsg.put_u32(RTT_ATTRIBUTE_TARGET_NUM_FTM_BURST,
                    config.num_frames_per_burst);
            if (result < 0) {
                return result;
            }
            ...
            result = mMsg.put_u32(RTT_ATTRIBUTE_TARGET_NUM_RETRY_FTMR,
                    config.num_retries_per_ftmr);
            if (result < 0) {
                return result;
            }

            result = mMsg.put_u32(RTT_ATTRIBUTE_TARGET_PERIOD,
                    config.burst_period);
            if (result < 0) {
                return result;
            }

            result = mMsg.put_u32(RTT_ATTRIBUTE_TARGET_BURST_DURATION,
                    config.burst_duration);
            if (result < 0) {
                return result;
            }

            result = mMsg.put_u8(RTT_ATTRIBUTE_TARGET_LCI,
                    config.LCI_request);
            if (result < 0) {
                return result;
            }

            result = mMsg.put_u8(RTT_ATTRIBUTE_TARGET_LCR,
                    config.LCR_request);
            if (result < 0) {
                return result;
            }

            result = mMsg.put_u8(RTT_ATTRIBUTE_TARGET_BW,
                    config.bw);
            if (result < 0) {
                return result;
            }

            result = mMsg.put_u8(RTT_ATTRIBUTE_TARGET_PREAMBLE,
                    config.preamble);
            if (result < 0) {
                return result;
            }
...
private:
    ...
    mac_addr* mAddrs;
public:
    RttCancelCommand(wifi_interface_handle iface, wifi_request_id id,
            unsigned int num_devices, mac_addr addr[])
        : WifiCommand("RttCancelCommand", iface, id), mNumDevices(num_devices),
          mAddrs(addr)
    {
        ...
        wifi_rtt_event_handler handler)
{
    if (iface == nullptr || num_rtt_config == 0) {
        ALOGE("%s: invalid args: iface=%p, num_rtt_config=%u", __FUNCTION__,
            (void *) iface, num_rtt_config);
        ...
        unsigned int num_devices, mac_addr addr[])
{
    if (iface == nullptr || num_devices == 0) {
        ALOGE("%s: invalid args: iface=%p, num_devices=%u", __FUNCTION__,
            (void *) iface, num_devices);
        return WIFI_ERROR_INVALID_ARGS;
    }
        ...
        wifi_rtt_capabilities *capabilities)
{
    if (iface == nullptr || capabilities == nullptr) {
        ALOGE("%s: invalid args: iface=%p, capabilities=%p", __FUNCTION__,
            (void *) iface, capabilities);
        return WIFI_ERROR_INVALID_ARGS;
    }
