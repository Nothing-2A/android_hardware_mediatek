...
public:
    ...
    MKeepAliveCommand(wifi_interface_handle iface, u8 id, u8 *ip_packet, u16 ip_packet_len,
        ...
        : WifiCommand("MKeepAliveCommand", iface, 0), mIndex(id), mIpPkt(ip_packet),
        ...
        u16 /*ether_type*/, u8 *ip_packet, u16 ip_packet_len, u8 *src_mac_addr, u8 *dst_mac_addr, u32 period_msec)
{
    if ((id <= 0 || id > N_AVAIL_ID) || iface == nullptr || ip_packet == nullptr
            || ip_packet_len > MKEEP_ALIVE_IP_PKT_MAX || src_mac_addr == nullptr || dst_mac_addr == nullptr
            || period_msec <= 0) {
        ALOGE("%s: invalid args: id=%d, iface=%p, ip_packet=%p, ip_packet_len=%d, src_mac_addr=%p, dst_mac_addr=%p,"
            "period_msec=%" PRIu32, __FUNCTION__, id, (void *) iface, ip_packet, ip_packet_len, src_mac_addr,
            dst_mac_addr, period_msec);
        return WIFI_ERROR_INVALID_ARGS;
    }

    MKeepAliveCommand *cmd = new MKeepAliveCommand(iface, id, ip_packet, ip_packet_len, src_mac_addr, dst_mac_addr,
