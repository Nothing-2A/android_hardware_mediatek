...
int WifiCommand::requestResponse(WifiRequest& request) {
    ...
    if (err)
        ALOGI("WifiCommand::requestResponse err=%d", err);
out:
    nl_cb_put(cb);
...
WifiVendorCommand::WifiVendorCommand(wifi_handle handle,
                                     ...
                                     u32 vendor_id,
                                     u32 subcmd)
        : WifiCommand("WifiVendorCommand", handle, id), mVendor_id(vendor_id), mSubcmd(subcmd),
        mVendorData(NULL), mDataLen(0)
{
    ALOGD("WifiVendorCommand %p created vendor_id:0x%x subcmd:%u",
          this, mVendor_id, mSubcmd);
}

WifiVendorCommand::WifiVendorCommand(const char *type,
                                     wifi_handle handle,
                                     wifi_request_id id,
                                     u32 vendor_id,
                                     u32 subcmd)
        : WifiCommand(type, handle, id), mVendor_id(vendor_id), mSubcmd(subcmd),
        mVendorData(NULL), mDataLen(0) {
    ALOGD("WifiVendorCommand %p created vendor_id:0x%x subcmd:%u",
          this, mVendor_id, mSubcmd);
}
...
wifi_error WifiVendorCommand::put_bytes(
                                int attribute, const char *data, int len)
{
