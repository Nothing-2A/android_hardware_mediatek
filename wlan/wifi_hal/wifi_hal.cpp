...
static wifi_error wifi_set_packet_filter(wifi_interface_handle handle, const u8 *program,
    u32 len);
...
static wifi_error wifi_set_scan_mode(const char * ifname, bool enable);
...
static wifi_error wifi_get_supported_iface_concurrency_matrix(wifi_handle handle,
...
public:
    ...
    {
        static_assert(sizeof(mIfaceNameCache) == sizeof(mIfaceInfo->name), "Inconsistent ifname buffer size");
    ...
    int createRequest(WifiRequest& request, int enable) {
        ...
        result = request.put_u32(WIFI_ATTRIBUTE_RSSI_MONITOR_MAX_RSSI,
                                (enable? mMax_rssi: 0));
        if (result < 0) {
            return result;
        }
        ALOGV("create request");
        result = request.put_u32(WIFI_ATTRIBUTE_RSSI_MONITOR_MIN_RSSI,
                                (enable? mMin_rssi: 0));
        if (result < 0) {
            return result;
        }
...
public:
    AddInterfaceCommand(wifi_handle handle, int ctrlIndex, const char* ifname, nl80211_iftype type)
        : WifiCommand("AddInterfaceCommand", handle, 0),
          mCtrlId(ctrlIndex), mIfName(ifname), mType(type) {}
...
public:
    MultiStaSetUseCaseCommand(wifi_handle handle, int ctrlIndex, int useCase)
        : WifiCommand("MultiStaSetUseCaseCommand", handle, 0), mCtrlId(ctrlIndex), mUseCase(useCase) {}

    virtual int create() {
        int ret = mMsg.create(GOOGLE_OUI, WIFI_SUBCMD_SET_MULTISTA_USE_CASE);
        if (ret < 0) {
            ALOGE("Can't create message %d to send to driver - %d", WIFI_SUBCMD_SET_MULTISTA_USE_CASE, ret);
            return ret;
        }
...
public:
    SetSubsystemRestartHandlerCommand(wifi_handle handle, wifi_subsystem_restart_handler eh)
        : WifiVendorCommand("SetSubsystemRestartHandlerCommand", handle, 0, OUI_MTK, MTK_EVENT_RESET_TRIGGERED),
          mHandler(eh)
    {}
...
class AndroidPacketFilterCommand : public WifiCommand {
    private:
        ...
        int mReqType;
    public:
        AndroidPacketFilterCommand(wifi_interface_handle handle,
                u32* version, u32* max_len)
            : WifiCommand("AndroidPacketFilterCommand", handle, 0),
                mProgram(nullptr), mReadProgram(nullptr), mProgramLen(0),
                mVersion(version), mMaxLen(max_len), mReqType(GET_APF_CAPABILITIES)
        {}

        AndroidPacketFilterCommand(wifi_interface_handle handle,
                const u8* program, u32 len)
            : WifiCommand("AndroidPacketFilterCommand", handle, 0),
                mProgram(program), mReadProgram(nullptr), mProgramLen(len),
                mVersion(nullptr), mMaxLen(nullptr), mReqType(SET_APF_PROGRAM)
        {}

        AndroidPacketFilterCommand(wifi_interface_handle handle,
            u8* host_dst, u32 len)
            : WifiCommand("AndroidPacketFilterCommand", handle, 0),
                mProgram(nullptr), mReadProgram(host_dst), mProgramLen(len),
                mVersion(nullptr), mMaxLen(nullptr), mReqType(READ_APF_PROGRAM)
        {
        }
 ...
 *         int trailing_array[];
 *     };
 *     DO NOT use `sizeof(AAA)` to calculate the offset of trailing_array. USE `offsetof(AAA, trailing_array)`.
 */
...
public:
    ...
    virtual int create() {
        ...
        if (ret < 0) {
            ALOGE("Can't create message %d to send to driver - %d", WIFI_LSTATS_SUBCMD_GET_INFO, ret);
        }
        return ret;
    }
...
private:
     ...
     * after this.
     */
    bool check_data_sanity(void *data, int len, uint32_t &num_radios, wifi_radio_stat *&radio_stats_ptr) {
        ...
        for (int i = 0; i < ml_stat->num_links; ++i) {
            ...
            ALOGV("LLS %s: %d: current_link_stat(%p)->num_peers=%" PRIu32, __FUNCTION__, __LINE__, current_link_stat, current_link_stat->num_peers);
            for (int j = 0; j < current_link_stat->num_peers; ++j) {
                ...
                ALOGV("LLS %s: %d: current_peer_info(%p)->num_rate=%" PRIu32, __FUNCTION__, __LINE__, current_peer_info, current_peer_info->num_rate);
                // trailing wifi_rate_stat(s)
                int expected_size =
                    offsetof(wifi_peer_info, rate_stats) + current_peer_info->num_rate * sizeof(wifi_rate_stat);
                if (confirmed_size + expected_size > len) {
                    ALOGE(
                        "LLS %s: %d: [FAILED] check trailing wifi_rate_stat(s), confirmed_size=%" PRIu32 ", expected_size=%d, len=%d",
        ...
        for (int i = 0; i < num_radios; ++i) {
            if (confirmed_size + offsetof(wifi_radio_stat, channels) > len) {
                ...
                ALOGE(
                    "LLS %s: %d: [FAILED] check wifi_radio_stat(s), confirmed_size=%" PRIu32 ", offsetof(wifi_radio_stat, channels)=%zu, len=%d",
                    __FUNCTION__, __LINE__, confirmed_size, offsetof(wifi_radio_stat, channels), len);
                return false;
            }
            ALOGI("LLS %s: %d: current_radio_stat->num_channels=%" PRIu32, __FUNCTION__, __LINE__, current_radio_stat->num_channels);
            int expected_size =
                offsetof(wifi_radio_stat, channels) + current_radio_stat->num_channels * sizeof(wifi_channel_stat);
            if (confirmed_size + expected_size > len) {
                delete[] tx_time_per_levels_pointer_list;
                delete[] tx_levels_offsets;
                ALOGE(
                    "LLS %s: %d: [FAILED] check wifi_channel_stat(s), confirmed_size=%" PRIu32 ", expected_size=%d, sizeof(wifi_channel_stat)=%zu, len=%d",
                    __FUNCTION__, __LINE__, confirmed_size, expected_size, sizeof(wifi_channel_stat), len);
                return false;
            }
    ...
    void dump_ml_stat(wifi_iface_ml_stat * ml_stat_ptr) {
        ...
        ALOGI("%04lx|iface|%p",
              ML_STAT_(iface),
              (void *)(ml_stat_ptr->iface));

        // info
        {
            ALOGI("%04lx|info.mode|%d",
                  ML_STAT_(info) + IFACE_INFO_STAT_(mode),
                  ml_stat_ptr->info.mode);
            ALOGI("%04lx|info.mac_addr|" MACSTR,
                  ML_STAT_(info) + IFACE_INFO_STAT_(mac_addr),
                  MAC2STR(ml_stat_ptr->info.mac_addr));
            ALOGI("%04lx|info.state|%d",
                  ML_STAT_(info) + IFACE_INFO_STAT_(state),
                  ml_stat_ptr->info.state);
            ALOGI("%04lx|info.roaming|%d",
                  ML_STAT_(info) + IFACE_INFO_STAT_(roaming),
                  ml_stat_ptr->info.roaming);
            ALOGI("%04lx|info.capabilities|%" PRIu32,
                  ML_STAT_(info) + IFACE_INFO_STAT_(capabilities),
                  ml_stat_ptr->info.capabilities);
            ALOGI("%04lx|info.ssid|%s",
                  ML_STAT_(info) + IFACE_INFO_STAT_(ssid),
                  ml_stat_ptr->info.ssid);
            ALOGI("%04lx|info.bssid|" MACSTR,
                  ML_STAT_(info) + IFACE_INFO_STAT_(bssid),
                  MAC2STR(ml_stat_ptr->info.bssid));
            ALOGI("%04lx|info.ap_country_str|%c%c%c",
                  ML_STAT_(info) + IFACE_INFO_STAT_(ap_country_str),
                  ml_stat_ptr->info.ap_country_str[0],
                  ml_stat_ptr->info.ap_country_str[1],
                  ml_stat_ptr->info.ap_country_str[2]);
            ALOGI("%04lx|info.country_str|%c%c%c",
                  ML_STAT_(info) + IFACE_INFO_STAT_(country_str),
                  ml_stat_ptr->info.country_str[0],
                  ml_stat_ptr->info.country_str[1],
        ...
        ALOGI("%04lx|num_links|%d",
              ML_STAT_(num_links),
              ml_stat_ptr->num_links);
        ...
        for (int i = 0; i < ml_stat_ptr->num_links; ++i) {
            ALOGI("%04lx|links[%d].link_id|%" PRIu8,
                  current_offset + LINK_STAT_(link_id),
                  i, link_ptr->link_id);
            ALOGI("%04lx|links[%d].state|%d",
                  current_offset + LINK_STAT_(state),
                  i, (int) link_ptr->state);
            ALOGI("%04lx|links[%d].radio|%d",
                  current_offset + LINK_STAT_(radio),
                  i, (int) link_ptr->radio);
            ALOGI("%04lx|links[%d].frequency|%" PRIu32,
                  current_offset + LINK_STAT_(frequency),
                  i, link_ptr->frequency);
            ALOGI("%04lx|links[%d].beacon_rx|%" PRIu32,
                  current_offset + LINK_STAT_(beacon_rx),
                  i, link_ptr->beacon_rx);
            ALOGI("%04lx|links[%d].average_tsf_offset|%" PRIu64,
                  current_offset + LINK_STAT_(average_tsf_offset),
                  i, link_ptr->average_tsf_offset);
            ALOGI("%04lx|links[%d].leaky_ap_detected|%" PRIu32,
                  current_offset + LINK_STAT_(leaky_ap_detected),
                  i, link_ptr->leaky_ap_detected);
            ALOGI("%04lx|links[%d].leaky_ap_avg_num_frames_leaked|%" PRIu32,
                  current_offset + LINK_STAT_(leaky_ap_avg_num_frames_leaked),
                  i, link_ptr->leaky_ap_avg_num_frames_leaked);
            ALOGI("%04lx|links[%d].leaky_ap_guard_time|%" PRIu32,
                  current_offset + LINK_STAT_(leaky_ap_guard_time),
                  i, link_ptr->leaky_ap_guard_time);
            ALOGI("%04lx|links[%d].mgmt_rx|%" PRIu32,
                  current_offset + LINK_STAT_(mgmt_rx),
                  i, link_ptr->mgmt_rx);
            ALOGI("%04lx|links[%d].mgmt_action_rx|%" PRIu32,
                  current_offset + LINK_STAT_(mgmt_action_rx),
                  i, link_ptr->mgmt_action_rx);
            ALOGI("%04lx|links[%d].mgmt_action_tx|%" PRIu32,
                  current_offset + LINK_STAT_(mgmt_action_tx),
                  i, link_ptr->mgmt_action_tx);
            ALOGI("%04lx|links[%d].rssi_mgmt|%d",
                  current_offset + LINK_STAT_(rssi_mgmt),
                  i, link_ptr->rssi_mgmt);
            ALOGI("%04lx|links[%d].rssi_data|%d",
                  current_offset + LINK_STAT_(rssi_data),
                  i, link_ptr->rssi_data);
            ALOGI("%04lx|links[%d].rssi_ack|%d",
                  current_offset + LINK_STAT_(rssi_ack),
                  i, link_ptr->rssi_ack);

            // wifi_wmm_ac_stat(s)
            for (int j = 0; j < WIFI_AC_MAX; ++j) {
                int ac_offset = current_offset + LINK_STAT_(ac) + sizeof(wifi_wmm_ac_stat) * j;
                ALOGI("%04lx|links[%d].ac[%d].ac|%d",
                      ac_offset + WMM_STAT_(ac),
                      i, j, link_ptr->ac[j].ac);
                ALOGI("%04lx|links[%d].ac[%d].tx_mpdu|%" PRIu32,
                      ac_offset + WMM_STAT_(tx_mpdu),
                      i, j, link_ptr->ac[j].tx_mpdu);
                ALOGI("%04lx|links[%d].ac[%d].rx_mpdu|%" PRIu32,
                      ac_offset + WMM_STAT_(rx_mpdu),
                      i, j, link_ptr->ac[j].rx_mpdu);
                ALOGI("%04lx|links[%d].ac[%d].tx_mcast|%" PRIu32,
                      ac_offset + WMM_STAT_(tx_mcast),
                      i, j, link_ptr->ac[j].tx_mcast);
                ALOGI("%04lx|links[%d].ac[%d].rx_mcast|%" PRIu32,
                      ac_offset + WMM_STAT_(rx_mcast),
                      i, j, link_ptr->ac[j].rx_mcast);
                ALOGI("%04lx|links[%d].ac[%d].rx_ampdu|%" PRIu32,
                      ac_offset + WMM_STAT_(rx_ampdu),
                      i, j, link_ptr->ac[j].rx_ampdu);
                ALOGI("%04lx|links[%d].ac[%d].tx_ampdu|%" PRIu32,
                      ac_offset + WMM_STAT_(tx_ampdu),
                      i, j, link_ptr->ac[j].tx_ampdu);
                ALOGI("%04lx|links[%d].ac[%d].mpdu_lost|%" PRIu32,
                      ac_offset + WMM_STAT_(mpdu_lost),
                      i, j, link_ptr->ac[j].mpdu_lost);
                ALOGI("%04lx|links[%d].ac[%d].retries|%" PRIu32,
                      ac_offset + WMM_STAT_(retries),
                      i, j, link_ptr->ac[j].retries);
                ALOGI("%04lx|links[%d].ac[%d].retries_short|%" PRIu32,
                      ac_offset + WMM_STAT_(retries_short),
                      i, j, link_ptr->ac[j].retries_short);
                ALOGI("%04lx|links[%d].ac[%d].retries_long|%" PRIu32,
                      ac_offset + WMM_STAT_(retries_long),
                      i, j, link_ptr->ac[j].retries_long);
                ALOGI("%04lx|links[%d].ac[%d].contention_time_min|%" PRIu32,
                      ac_offset + WMM_STAT_(contention_time_min),
                      i, j, link_ptr->ac[j].contention_time_min);
                ALOGI("%04lx|links[%d].ac[%d].contention_time_max|%" PRIu32,
                      ac_offset + WMM_STAT_(contention_time_max),
                      i, j, link_ptr->ac[j].contention_time_max);
                ALOGI("%04lx|links[%d].ac[%d].contention_time_avg|%" PRIu32,
                      ac_offset + WMM_STAT_(contention_time_avg),
                      i, j, link_ptr->ac[j].contention_time_avg);
                ALOGI("%04lx|links[%d].ac[%d].contention_num_samples|%" PRIu32,
                      ac_offset + WMM_STAT_(contention_num_samples),
                      i, j, link_ptr->ac[j].contention_num_samples);
            } // wifi_wmm_ac_stat(s)

            ALOGI("%04lx|links[%d].time_slicing_duty_cycle_percent|%" PRIu8,
                  current_offset + LINK_STAT_(time_slicing_duty_cycle_percent),
                  i, link_ptr->time_slicing_duty_cycle_percent);

            ALOGI("%04lx|links[%d].num_peers|%" PRIu32,
                  current_offset + LINK_STAT_(num_peers),
                  i, link_ptr->num_peers);
            ...
            for (int j = 0; j < link_ptr->num_peers; ++j) {
                ALOGI("%04lx|links[%d].peer_info[%d].type|%d",
                      current_offset + PEER_INFO_(type),
                      i, j, peer_info_ptr->type);
                ALOGI("%04lx|links[%d].peer_info[%d].peer_mac_address|" MACSTR,
                      current_offset + PEER_INFO_(peer_mac_address),
                      i, j, MAC2STR(peer_info_ptr->peer_mac_address));
                ALOGI("%04lx|links[%d].peer_info[%d].capabilities|%" PRIu32,
                      current_offset + PEER_INFO_(capabilities),
                      i, j, peer_info_ptr->capabilities);
                    // peer_info[%d].bssload
                    {
                        ALOGI("%04lx|links[%d].peer_info[%d].bssload.sta_count|%" PRIu16,
                              current_offset + PEER_INFO_(bssload) + BSSLOAD_INFO_(sta_count),
                              i, j, peer_info_ptr->bssload.sta_count);
                        ALOGI("%04lx|links[%d].peer_info[%d].bssload.chan_util|%" PRIu16,
                              current_offset + PEER_INFO_(bssload) + BSSLOAD_INFO_(chan_util),
                              i, j, peer_info_ptr->bssload.chan_util);
                    } // peer_info[%d].bssload
                ALOGI("%04lx|links[%d].peer_info[%d].num_rate|%" PRIu32,
                      current_offset + PEER_INFO_(num_rate),
                      i, j, peer_info_ptr->num_rate);
                ...
                for (int k = 0; k < peer_info_ptr->num_rate; ++k) {
                    ...
                    // peer_info[%d].rate_stats[%d].rate
                    {
                        ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].rate.preamble(3)|%01x",
                              current_offset + RATE_STAT_(rate),
                              i, j, k, rate_stat_ptr->rate.preamble);
                        ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].rate.nss(2)|%01x",
                              current_offset + RATE_STAT_(rate),
                              i, j, k, rate_stat_ptr->rate.nss);
                        ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].rate.bw(3)|%01x",
                              current_offset + RATE_STAT_(rate),
                              i, j, k, rate_stat_ptr->rate.bw);
                        ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].rate.rateMcsIdx(8)|%01x",
                              current_offset + RATE_STAT_(rate),
                              i, j, k, rate_stat_ptr->rate.rateMcsIdx);
                        ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].rate.bitrate|%" PRIu32,
                              current_offset + RATE_STAT_(rate) + RATE_(bitrate),
                              i, j, k, rate_stat_ptr->rate.bitrate);
                    } // peer_info[%d].rate_stats[%d].rate
                    ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].tx_mpdu|%" PRIu32,
                          current_offset + RATE_STAT_(tx_mpdu),
                          i, j, k, rate_stat_ptr->tx_mpdu);
                    ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].rx_mpdu|%" PRIu32,
                          current_offset + RATE_STAT_(rx_mpdu),
                          i, j, k, rate_stat_ptr->rx_mpdu);
                    ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].mpdu_lost|%" PRIu32,
                          current_offset + RATE_STAT_(mpdu_lost),
                          i, j, k, rate_stat_ptr->mpdu_lost);
                    ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].retries|%" PRIu32,
                          current_offset + RATE_STAT_(retries),
                          i, j, k, rate_stat_ptr->retries);
                    ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].retries_short|%" PRIu32,
                          current_offset + RATE_STAT_(retries_short),
                          i, j, k, rate_stat_ptr->retries_short);
                    ALOGI("%04lx|links[%d].peer_info[%d].rate_stats[%d].retries_long|%" PRIu32,
                          current_offset + RATE_STAT_(retries_long),
                          i, j, k, rate_stat_ptr->retries_long);
            ...
            wifi_iface_ml_stat * ml_stat_ptr)
    {
        ...
        for (int i = 0; i < num_radios; ++i) {
            ALOGI("%04lx|wifi_radio_stat[%d].radio|%d",
                  current_offset + RADIO_STAT_(radio),
                  i, current->radio);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time),
                  i, current->on_time);
            ALOGI("%04lx|wifi_radio_stat[%d].tx_time|%" PRIu32,
                  current_offset + RADIO_STAT_(tx_time),
                  i, current->tx_time);
            ALOGI("%04lx|wifi_radio_stat[%d].num_tx_levels|%" PRIu32,
                  current_offset + RADIO_STAT_(num_tx_levels),
                  i, current->num_tx_levels);
            ALOGI("%04lx|wifi_radio_stat[%d].tx_time_per_levels|%p",
                  current_offset + RADIO_STAT_(tx_time_per_levels),
                  i, current->tx_time_per_levels);
            for (int j = 0; j < current->num_tx_levels; ++j) {
                ALOGI("....|wifi_radio_stat[%d].tx_time_per_levels[%d]|%" PRIu32,
                  i, j, current->tx_time_per_levels[j]);
            }
            ALOGI("%04lx|wifi_radio_stat[%d].rx_time|%" PRIu32,
                  current_offset + RADIO_STAT_(rx_time),
                  i, current->rx_time);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time_scan|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time_scan),
                  i, current->on_time_scan);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time_nbd|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time_nbd),
                  i, current->on_time_nbd);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time_gscan|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time_gscan),
                  i, current->on_time_gscan);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time_roam_scan|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time_roam_scan),
                  i, current->on_time_roam_scan);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time_pno_scan|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time_pno_scan),
                  i, current->on_time_pno_scan);
            ALOGI("%04lx|wifi_radio_stat[%d].on_time_hs20|%" PRIu32,
                  current_offset + RADIO_STAT_(on_time_hs20),
                  i, current->on_time_hs20);
            ALOGI("%04lx|wifi_radio_stat[%d].num_channels|%" PRIu32,
                  current_offset + RADIO_STAT_(num_channels),
                  i, current->num_channels);
            ...
            for (int j = 0; j < current->num_channels; ++j) {
                // wifi_radio_stat[%d].channels[%d].channel
                {
                    ALOGI("%04lx|wifi_radio_stat[%d].channels[%d].channel.width|%d",
                          current_offset + CHANNEL_STAT_(channel) + CHANNEL_INFO_(width),
                          i, j, current->channels[j].channel.width);
                    ALOGI("%04lx|wifi_radio_stat[%d].channels[%d].channel.center_freq|%d",
                          current_offset + CHANNEL_STAT_(channel) + CHANNEL_INFO_(center_freq),
                          i, j, current->channels[j].channel.center_freq);
                    ALOGI("%04lx|wifi_radio_stat[%d].channels[%d].channel.center_freq0|%d",
                          current_offset + CHANNEL_STAT_(channel) + CHANNEL_INFO_(center_freq0),
                          i, j, current->channels[j].channel.center_freq0);
                    ALOGI("%04lx|wifi_radio_stat[%d].channels[%d].channel.center_freq1|%d",
                          current_offset + CHANNEL_STAT_(channel) + CHANNEL_INFO_(center_freq1),
                          i, j, current->channels[j].channel.center_freq1);
                } // wifi_radio_stat[%d].channels[%d].channel
                ALOGI("%04lx|wifi_radio_stat[%d].channels[%d].on_time|%" PRIu32,
                      current_offset + CHANNEL_STAT_(on_time),
                      i, j, current->channels[j].on_time);
                ALOGI("%04lx|wifi_radio_stat[%d].channels[%d].cca_busy_time|%" PRIu32,
                    current_offset + CHANNEL_STAT_(cca_busy_time),
                      i, j, current->channels[j].cca_busy_time);
...
public:
    GetSupportedRadioCombinationsMatrixCommand(wifi_handle handle, int ctrl_iface,
        u32 max_size, u32* size, wifi_radio_combination_matrix *radio_combination_matrix)
        : WifiCommand("GetSupportedRadioCombinationsMatrixCommand", handle, 0),
          ctrl_iface(ctrl_iface), matrix(radio_combination_matrix), rc_size(size),
          set_size_max(max_size)
    {
    }
    ...
    virtual int create() {
        ...
        if (ret < 0) {
            ALOGE("Can't create message %d to send to driver - %d", MTK_SUBCMD_GET_RADIO_COMBO_MATRIX, ret);
        }
            ...
            if (it.get_type() == WIFI_ATTRIBUTE_RADIO_COMBINATIONS_MATRIX_MATRIX) {
                ...
                for (int i = 0; i < COMB_MATRIX_LEN; ++i) {
                    if (driver_matrix->comb_mtx[i].band_0 != 0) {
                        ...
                        if (NL_OK != convert_antenna(driver_matrix->comb_mtx[i].ant_0,
                        ...
                        if (driver_matrix->comb_mtx[i].band_1 != 0) {
                            ...
                            if (NL_OK != convert_antenna(driver_matrix->comb_mtx[i].ant_1,
                                                    &(p_combo->radio_configurations[1].antenna_cfg))) {
                                return NL_SKIP;
                            }
                        }
                        p_combo = (wifi_radio_combination *)
                            ((uint8_t*)(p_combo) + sizeof(wifi_radio_combination)
                                + sizeof(wifi_radio_configuration) * p_combo->num_radio_configurations);
                    }
                }
            } else {
                ALOGW("Ignoring invalid attribute type = %d, size = %d",
    ...
    void dump_matrix(wifi_radio_combination_matrix *matrix) {
        ...
        for (int i = 0; i < matrix->num_radio_combinations; ++i) {
            ...
            for (int j = 0; j < p_combo->num_radio_configurations; ++j) {
                ALOGI("  band=%d, antenna=%d",
                    p_combo->radio_configurations[j].band,
...
public:
    GetUsableChannelCommand(wifi_handle handle, int ctrl_iface, u32 band_mask, u32 iface_mode_mask,
                u32 filter_mask, u32 max_size, u32* size, wifi_usable_channel* channels)
        : WifiCommand("GetUsableChannelCommand", handle, 0), mCtrlIface(ctrl_iface), mBandMask(band_mask),
                mIfaceModeMask(iface_mode_mask), mFilterMask(filter_mask), mMaxSize(max_size),
    ...
    virtual int handleResponse(WifiEvent& reply) {
        ...
        if (it.get_type() == WIFI_ATTRIBUTE_USABLE_CHANNEL_RESP_ARRAY) {
            ...
            if (it.get_len() <
                    sizeof(wifi_usable_channel) * resp_data->array_size + sizeof(wifi_usable_channel_response)) {
                ALOGE("Invalid response size: %d, array size: %" PRIu16, it.get_len(), resp_data->array_size);
                return NL_SKIP;
            }
            *mSize = resp_data->array_size;
            memcpy(channels, resp_data->channel_array, sizeof(wifi_usable_channel) * resp_data->array_size);
        } else {
            ALOGE("Unknown attribute: %d expecting %d",
                it.get_type(), WIFI_ATTRIBUTE_USABLE_CHANNEL_RESP_ARRAY);
            return NL_SKIP;
        }
...
public:
    GetChipCapabilitiesCommand(wifi_handle handle, int ctrl_iface, wifi_chip_capabilities* capabilities)
        : WifiCommand("GetChipCapabilitiesCommand", handle, 0), mCtrlIface(ctrl_iface), mCapabilities(capabilities) {}

    virtual int create() {
        ALOGV("GetChipCapabilitiesCommand::create");
        int ret = mMsg.create(OUI_MTK, MTK_SUBCMD_GET_CHIP_CAPABILITIES);
        if (ret < 0) {
            ALOGE("Can't create message %d to send to driver - %d", MTK_SUBCMD_GET_CHIP_CAPABILITIES, ret);
        }
    ...
    virtual int handleResponse(WifiEvent& reply) {
        ...
        nl_iterator it(vendor_data);
        for (nl_iterator it(vendor_data); it.has_next(); it.next()) {
            if (it.get_type() == WIFI_ATTRIBUTE_CHIP_CAPABILITIES_RESP_MAX_MLO_ASSOCIATION_LINK_COUNT) {
                mCapabilities->max_mlo_association_link_count = it.get_u32();
            } else if (it.get_type() == WIFI_ATTRIBUTE_CHIP_CAPABILITIES_RESP_MAX_MLO_STR_LINK_COUNT ) {
                mCapabilities->max_mlo_str_link_count = it.get_u32();
            } else if (it.get_type() == WIFI_ATTRIBUTE_CHIP_CAPABILITIES_RESP_MAX_CONCURRENT_TDLS_SESSION_COUNT ) {
...
public:
    GetChipConcurrencyMatrixCommand(wifi_handle handle, int ctrl_iface, wifi_iface_concurrency_matrix *matrix)
        : WifiCommand("GetChipConcurrencyMatrixCommand", handle, 0), mCtrlIface(ctrl_iface), mMatrix(matrix) {}
...
wifi_error wifi_get_iface_name(wifi_interface_handle iface, char *name, size_t size)
{
    if (iface == nullptr || name == nullptr) {
        ALOGE("%s: invalid args: iface=%p, name=%p",
            __FUNCTION__, (void*) iface, name);
...
wifi_error wifi_get_supported_feature_set(wifi_interface_handle iface, feature_set *pset)
{
    if (iface == nullptr || pset == nullptr) {
        ALOGE("%s: invalid args: iface=%p, pset=%p",
            __FUNCTION__, (void*) iface, pset);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
wifi_error wifi_set_country_code(wifi_interface_handle iface, const char *country_code)
{
    if (iface == nullptr || country_code == nullptr) {
        ALOGE("%s: invalid args: iface=%p, country_code=%p",
            __FUNCTION__, (void *)iface, country_code);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
wifi_error wifi_get_roaming_capabilities(wifi_interface_handle iface, wifi_roaming_capabilities *caps)
{
    if (iface == nullptr) {
        ALOGE("%s: invalid args: iface=%p", __FUNCTION__, (void*) iface);
        return WIFI_ERROR_INVALID_ARGS;
...
wifi_error wifi_configure_roaming(wifi_interface_handle iface, wifi_roaming_config *roaming_config)
{
    if (iface == nullptr || roaming_config == nullptr) {
        ALOGE("%s: invalid args: iface=%p, roaming_config=%p",
            __FUNCTION__, (void*) iface, roaming_config);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
{
    if (iface == nullptr) {
        ALOGE("%s: invalid args: iface=%p",
            __FUNCTION__, (void*) iface);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
wifi_error wifi_get_valid_channels(wifi_interface_handle iface,
        int band, int max_channels, wifi_channel *channels, int *num_channels)
{
    if (iface == nullptr || channels == nullptr || num_channels == nullptr) {
        ALOGE("%s: invalid args: iface=%p, channels=%p, num_channels=%p",
            __FUNCTION__, (void*) iface, channels, num_channels);
        return WIFI_ERROR_INVALID_ARGS;
    }
    ...
    WLAN_DRIVER_WAKE_REASON_CNT *wifi_wake_reason_cnt)
{
    if (iface == nullptr || wifi_wake_reason_cnt == nullptr) {
        ALOGE("%s: invalid args: iface=%p, wifi_wake_reason_cnt=%p",
            __FUNCTION__, (void*) iface, wifi_wake_reason_cnt);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
static wifi_error wifi_get_packet_filter_capabilities(wifi_interface_handle iface,
        u32 *version, u32 *max_len)
{
    if (iface == nullptr || version == nullptr || max_len == nullptr) {
        ALOGE("%s: invalid args: iface=%p, version=%p, max_len=%p",
            __FUNCTION__, (void*) iface, version, max_len);
        return WIFI_ERROR_INVALID_ARGS;
    }
        ...
        const u8 *program, u32 len)
{
    if (iface == nullptr || program == nullptr || len == 0) {
        ALOGE("%s: invalid args: iface=%p, program=%p, len=%d",
            __FUNCTION__, (void*) iface, program, (int) len);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
static wifi_error wifi_read_packet_filter(wifi_interface_handle iface,
    u32 src_offset, u8 *host_dst, u32 length)
{
    if (iface == nullptr || host_dst == nullptr || length == 0) {
        ALOGE("%s: invalid args: iface=%p, host_dst=%p, length=%d",
            __FUNCTION__, (void*) iface, host_dst, (int) length);
        return WIFI_ERROR_INVALID_ARGS;
    }
    ...
    wifi_tx_report *tx_report_bufs, size_t n_requested_fates, size_t *n_provided_fates)
{
    if (iface == nullptr || tx_report_bufs == nullptr || n_provided_fates == nullptr) {
        ALOGE("%s: invalid args: iface=%p, tx_report_bufs=%p, n_provided_fates=%p",
            __FUNCTION__, (void*) iface, tx_report_bufs, n_provided_fates);
        return WIFI_ERROR_INVALID_ARGS;
    }
        ...
        wifi_rx_report *rx_report_bufs, size_t n_requested_fates, size_t *n_provided_fates)
{
    if (iface == nullptr || rx_report_bufs == nullptr || n_provided_fates == nullptr) {
        ALOGE("%s: invalid args: iface=%p, rx_report_bufs=%p, n_provided_fates=%p",
            __FUNCTION__, (void*) iface, rx_report_bufs, n_provided_fates);
        return WIFI_ERROR_INVALID_ARGS;
    }
        ...
        u32 *num_rings, wifi_ring_buffer_status *status)
{
    if (iface == nullptr || num_rings == nullptr || status == nullptr) {
        ALOGE("%s: invalid args: iface=%p, num_rings=%p, status=%p",
            __FUNCTION__, (void*) iface, num_rings, status);
        return WIFI_ERROR_INVALID_ARGS;
    }
        ...
        unsigned int *support)
{
    if (iface == nullptr || support == nullptr) {
        ALOGE("%s: invalid args: iface=%p, support=%p",
            __FUNCTION__, (void*) iface, support);
        return WIFI_ERROR_INVALID_ARGS;
    }
        ...
        wifi_interface_type iface_type)
{
    if (!handle || ifname == nullptr) {
        ALOGE("%s: invalid args: handle=%p, ifname=%p",
                __FUNCTION__, (void *) handle, ifname);
        return WIFI_ERROR_INVALID_ARGS;
    }
...
wifi_interface_handle wifi_get_iface_handle(wifi_handle handle, char *name)
{
    if (handle == nullptr || name == nullptr) {
        ALOGE("%s: invalid args: handle=%p, name=%p",
            __FUNCTION__, (void*) handle, name);
        return NULL;
    }
    hal_info *info = (hal_info *)handle;
    for (int i = 0; i < info->num_interfaces ; i++) {
        ...
        u32 *stats_clear_rsp_mask, u8 stop_req, u8 *stop_rsp)
{
    ...
    if (iface == nullptr || stats_clear_rsp_mask == nullptr || stop_rsp == nullptr) {
        ALOGE("%s: invalid args: iface=%p, stats_clear_rsp_mask=%p, stop_rsp=%p",
            __FUNCTION__, (void*) iface, stats_clear_rsp_mask, stop_rsp);
        return WIFI_ERROR_INVALID_ARGS;
    }
    ...
    u32 max_size, u32* size, wifi_radio_combination_matrix *radio_combination_matrix)
{
    ...
    GetSupportedRadioCombinationsMatrixCommand command(
        handle, ctrl_iface_index, max_size, size, radio_combination_matrix);
    return (wifi_error) command.requestResponse();
}
...
wifi_error wifi_set_scan_mode(const char* ifname, bool enable)
{
    ...
    if (ioctl_sock < 0) {
        ALOGE("%s: failed to create socket: ret=%d, errno=%s",
            __FUNCTION__, ioctl_sock, strerror(errno));
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    close(ioctl_sock);
    if (ret < 0) {
        ALOGE("%s: ioctl: cmd=%s, ret=%d, error=%s", __FUNCTION__, priv_cmd.buf, ret, strerror(errno));
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    u32 filter_mask, u32 max_size, u32* size, wifi_usable_channel* channels)
{
    if (handle == nullptr || size == nullptr || channels == nullptr) {
        ALOGE("%s: invalid args: handle=%p, size=%p, channels=%p",
            __FUNCTION__, handle, size, channels);
        return WIFI_ERROR_INVALID_ARGS;
    }
    ...
    GetUsableChannelCommand command(
        handle, ctrl_iface_index, band_mask, iface_mode_mask, filter_mask, max_size, size, channels);
    return (wifi_error) command.start();
}
        ...
        wifi_chip_capabilities *chip_capabilities)
{
    if (handle == nullptr || chip_capabilities == nullptr) {
        ALOGE("%s: invalid args: handle=%p, chip_capabilities=%p", __FUNCTION__, handle, chip_capabilities);
        return WIFI_ERROR_INVALID_ARGS;
    }
