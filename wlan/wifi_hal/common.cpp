...
wifi_error wifi_register_handler(wifi_handle handle, int cmd, nl_recvmsg_msg_cb_t func, void *arg)
{
    ...
    if (info->num_event_cb < info->alloc_event_cb) {
        ...
        ALOGI("Successfully added event handler %p:%p for command %d at %d",
            func, arg, cmd, info->num_event_cb);
        result = WIFI_SUCCESS;
    }
...
wifi_error wifi_register_vendor_handler(wifi_handle handle,
        uint32_t id, int subcmd, nl_recvmsg_msg_cb_t func, void *arg, char *ifname)
{
    ALOGI("%s: id=%d, subcmd=%d, ifname=%p [%s]", __FUNCTION__,
        id, subcmd, ifname, ifname == nullptr ? "<nullptr>" : ifname);
    hal_info *info = (hal_info *)handle;
    ...
    for (int i = 0; i < info->num_event_cb; i++) {
        ...
        if(cbi->vendor_id  == id && cbi->vendor_subcmd == subcmd) {
            ...
            if (ifname != nullptr && 0 != strncmp(cbi->ifname, ifname, sizeof(cbi->ifname) - 1)) {
                ALOGI("[%d] ifname mismatch, ignore: cbi->ifname [%s], ifname [%s]", i, cbi->ifname, ifname);
                continue;
            }
            cbi->cb_func = func;
            cbi->cb_arg  = arg;
            ALOGI("Update vendor handler %p:%p for vendor %d, subcmd %d and iface [%s] at %d",
                func, arg, id, subcmd, cbi->ifname, i);
            pthread_mutex_unlock(&info->cb_lock);
            return WIFI_SUCCESS;
        }
    }
    ...
    if (info->num_event_cb < info->alloc_event_cb) {
        ...
        info->num_event_cb++;
        ALOGI("Register vendor handler %p:%p for vendor %d, subcmd %d and iface [%s] at %d",
            arg, func, id, subcmd, ifname == nullptr ? "<nullptr>" : ifname, info->num_event_cb);
        result = WIFI_SUCCESS;
    }
...
void wifi_unregister_handler(wifi_handle handle, int cmd)
{
    ...
    for (int i = 0; i < info->num_event_cb; i++) {
        if (info->event_cb[i].nl_cmd == cmd) {
            ALOGI("Deregister wifi handler %p:%p for cmd = %d from %d",
                info->event_cb[i].cb_func, info->event_cb[i].cb_arg, cmd, i);
...
{
    ALOGI("%s: handle=%p, id=%d, subcmd=%d, ifname=%p [%s]", __FUNCTION__,
        handle, id, subcmd, ifname, ifname == nullptr ? "<nullptr>" : ifname);
    hal_info *info = (hal_info *)handle;
    ...
    for (int i = 0; i < info->num_event_cb; i++) {
        cb_info *cbi = &info->event_cb[i];
        ALOGI("%s: nl_cmd=%d, id=%d, subcmd=%d", __FUNCTION__, cbi->nl_cmd, cbi->vendor_id, cbi->vendor_subcmd);
        if (cbi->nl_cmd == NL80211_CMD_VENDOR && cbi->vendor_id == id && cbi->vendor_subcmd == subcmd) {
            // check ifname if specified
            if (ifname != nullptr && 0 != strncmp(cbi->ifname, ifname, sizeof(cbi->ifname) - 1)) {
                ALOGI("[%d] ifname mismatch, ignore: cbi->ifname [%s], ifname [%s]", i, cbi->ifname, ifname);
                continue;
            }
            ALOGI("Deregister vendor handler %p:%p for vendor %d, subcmd %d from %d",
                cbi->cb_arg, cbi->cb_func, id, subcmd, i);
            memmove(&info->event_cb[i], &info->event_cb[i+1], (info->num_event_cb - i - 1) * sizeof(cb_info));
            info->num_event_cb--;
            break;
        }
    }
...
void hexdump(void *buf, u16 len)
{
    ...
    if (len) {
        ...
        for (i = 0; ((i + 7) < len); i += 8) {
            ALOGE("%016" PRIx64 " | %02x %02x %02x %02x   %02x %02x %02x %02x",
                addr + i,
        ...
        if (remain > 0) {
            ...
            int result = snprintf(remain_buf, sizeof(remain_buf),
                "%016" PRIx64 " | %02x %02x %02x %02x   %02x %02x %02x",
                addr + i,
                (int) bytes[i],
